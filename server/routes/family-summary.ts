...
export const familySummary = new Hono()

/**
 * 要約そのものの読み書き。書き換えるのはここだけで、AI には触らせない。
 */
familySummary.on('GET', familyTopicPaths('/summary'), async (c) => {
  const { user, ref } = await requireFamilyTopic(c)
  return c.json<Summary>({ summary: await readSummary(user, ref) })
})

familySummary.on('PUT', familyTopicPaths('/summary'), async (c) => {
  const { user, ref } = await requireFamilyTopic(c)
 ...
 */
familySummary.on('POST', familyTopicPaths('/summary'), async (c) => {
  const { user, ref } = await requireFamilyTopic(c)
    ...
    try {
      const text = await collectAgent(
        choice,
        {
          ...
          prompt,
          systemPrompt: familySummarySystemPrompt({ topicName: meta.name, isGroup }),
