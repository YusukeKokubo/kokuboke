...
3 回目と同じタイミングでタグも一度付ける。タグは大分類だけを使う。会話の見出しや
一度きりの出来事は付けない。「大英博物館展予習」なら「美術館博物館巡り」で、
「大英博物館展」は作らない。既にある大分類で足りるならそれを使う。新しいタグを
足すのは、これから何度も話しそうな大分類がまだ無いときだけ。無いタグなら空の
`tags/{タグ名}.md` を作り、絵文字と棚を `tags.json` に残す。
人が雑な名前を付けていても、付け直すと大分類へ置き換えてよい。日常の自動 1 回では、
...
どちらも管理画面（`/admin`）の「Claude Code のログイン」「Cursor のログイン」からも通せる。
押すとサーバーが `claude auth login` / `cursor-agent login` を起こして認証の URL を出すので、
それをどの端末で開いてもよい。Claude Code は認証の最後にコードが出るので、画面の欄に
貼って送る。cursor は認証するだけで済む。済むと画面がログイン中のアカウントに変わる。
版を上げてログインが切れたときも、SSH は要らない。
...
本番のデータで試すときは、`npm run data:pull` で NAS から手元へ写してから、写しの方を
`DATA_DIR` に向ける。写し元は `.env` の `DATA_PULL_FROM`。SMB の先を直接 `DATA_DIR` に
すると、CLI が作業ディレクトリを SMB 越しに探るので、一文字目まで数十秒かかる。
写しは片方向で、手元で作った会話や直したファイルは次に写したときに消える（本番には出ない）。
取り違えて本番を消さないよう、写し先は目印の `.kokuboke-pull` があるか空のフォルダに限る。

写しがリポジトリの中や、上の階層に clone（compose を置いている `kokuboke/`）がある場所だと、
そこのプロジェクト用 `AGENTS.md` も一緒に読まれ、人格と開発の指示が混ざる。
clone に `CLAUDE.md` があった頃はそちらだけが読まれて人格が効かなかった。
コンテナでは `/data` に直接マウントされるので起きない。
